...
namespace aiko
{
    ...
    ThreadSystem::ThreadSystem(int initialThreadCount)
        : stopping(false)
        , finished(false)
    {
        ...
        for (int loop = 0; loop < initialThreadCount; ++loop)
        {
            ...
            */
            ...
            workers.emplace_back(&ThreadSystem::workerAction, this);
        }
        /*
    ...
    void ThreadSystem::tellAllThreadsToStop()
    {
        std::unique_lock<std::mutex> locker(lock);
        stopping = true;
        /*
    ...
    void ThreadSystem::addWorkItem(std::function<void()> action)
    {
        std::unique_lock<std::mutex> locker(lock);
        if (stopping) {
    ...
    void ThreadSystem::workerAction()
    {
        while (!finished)
        {
    ...
    std::function<void()> ThreadSystem::getNextWorkItem()
    {
        std::unique_lock<std::mutex> locker(lock);
